...
================================================================ */
const GuideNameModal = ({ visible, title, subtitle, value, onChangeText, error, onSave, onClose }) => (
    <Modal visible={visible} transparent animationType="fade" statusBarTranslucent onRequestClose={onClose}>
        <View style={styles.modalOverlay}>
            <View style={styles.modalCard}>
                ...
                <TouchableOpacity style={styles.saveButton} activeOpacity={0.85} onPress={onSave}>
...
const GroupNameModal = ({ visible, value, onChangeText, error, onSave, onClose }) => (
    <Modal visible={visible} transparent animationType="fade" statusBarTranslucent onRequestClose={onClose}>
        <View style={styles.modalOverlay}>
            <View style={styles.modalCard}>
                <View style={styles.modalHeader}>
                    <View style={{ flex: 1 }}>
                        ...
                        <Text style={styles.modalSubtitle}>Enter a new name for this group.</Text>
                    </View>
                    <TouchableOpacity style={styles.modalCloseBtn} onPress={onClose} activeOpacity={0.7}>
                        <X size={18} color="#6B7280" />
                    </TouchableOpacity>
    ...
    <Modal visible={visible} transparent animationType="fade" statusBarTranslucent onRequestClose={onClose}>
        <TouchableOpacity style={styles.menuOverlay} activeOpacity={1} onPress={onClose}>
            <View style={styles.groupMenuCard}>
                <TouchableOpacity style={styles.groupMenuItem} activeOpacity={0.7} onPress={onRename}>
                ...
                <TouchableOpacity style={styles.groupMenuItem} activeOpacity={0.7} onPress={onDelete}>
                    <Trash2 size={15} color="#EF4444" style={{ marginRight: 10 }} />
                    <Text style={[styles.groupMenuItemText, { color: '#EF4444' }]}>Delete</Text>
                </TouchableOpacity>
            </View>
        </TouchableOpacity>
    </Modal>
...
export default function OrderGuideScreen({ navigation }) {
    ...
    const saveGuides = async (updatedGuides) => {
        ...
        } catch (e) { console.error('Failed to save guides:', e); }
    };
    ...
    const getGroup = (guideId, groupId) => getGuide(guideId)?.groups.find(gr => gr.id === groupId);

    /* CRUD: Guide */
    const createGuide = () => {
        if (!modalInput.trim()) { setModalError('Please enter a name for your order guide.'); return; }
        ...
        const newGuide = {
            ...
            expanded: false,
            groups: [{ id: Date.now().toString() + '_g', name: 'Default Group', items: [] }],
        };
    ...
    const renameGuide = () => {
        if (!modalInput.trim()) { setModalError('Please enter a name.'); return; }
    ...
    const toggleExpand = (guideId) => {
        const updated = guides.map(g => g.id === guideId ? { ...g, expanded: !g.expanded } : g);
        setGuides(updated);
        saveGuides(updated);
    };
    ...
    const addGroup = (guideId) => {
        ...
        const updated = guides.map(g =>
            g.id === guideId
                ? { ...g, groups: [...g.groups, newGroup], updatedAt: new Date().toISOString() }  // ← stamp
                : g
        );
        setGuides(updated);
        saveGuides(updated);
    ...
    const Header = () => (
        <View style={styles.header}>
            ...
            <View style={styles.headerTitleWrap}>
                ...
                <Text style={styles.headerSubtitle}>Build and review guided orders</Text>
            </View>
        ...
        <View style={styles.contentWrap}>
            <View style={styles.emptyCard}>
                <View style={styles.starCircle}>
                    <Star size={32} color="#2563EB" strokeWidth={1.8} />
                </View>
                ...
                <TouchableOpacity style={styles.createButton} activeOpacity={0.85} onPress={() => openModal('createGuide')}>
    ...
    const DetailScreen = () => (
        <ScrollView contentContainerStyle={styles.detailScroll} showsVerticalScrollIndicator={false}>
            <View style={styles.breadcrumbRow}>
                <TouchableOpacity style={styles.backBtn} onPress={() => setScreen('list')} activeOpacity={0.7}>
            ...
            <View style={styles.detailTitleRow}>
                ...
                <View style={styles.itemsBadge}>
                    <Text style={styles.itemsBadgeText}>{activeGroup?.items.length} items</Text>
                </View>
            </View>
    ...
    const ListScreen = () => (
        <ScrollView contentContainerStyle={styles.listScroll} showsVerticalScrollIndicator={false}>
            <View style={styles.listHeaderRow}>
                ...
                <TouchableOpacity style={styles.listCreateBtn} activeOpacity={0.85} onPress={() => openModal('createGuide')}>
            ...
            {guides.map((guide) => (
                <View
                    key={guide.id}
                    style={[styles.guideCard, { marginBottom: 12 }]}
                >
                    <View style={styles.guideCardHeader}>
                        <GripVertical size={16} color="#9CA3AF" style={{ marginRight: 6 }} />
                        ...
                        <TouchableOpacity onPress={() => toggleExpand(guide.id)} style={{ marginLeft: 'auto', padding: 8 }} activeOpacity={0.7}>
                            {guide.expanded ? <ChevronDown size={18} color="#6B7280" /> : <ChevronRight size={18} color="#6B7280" />}
                        </TouchableOpacity>
                    </View>

                    {/* UPDATED TIME — uses getRelativeTime() on stored updatedAt */}
                    <Text style={styles.guideCardMeta}>
                        {guide.groups.reduce((a, g) => a + g.items.length, 0)} items · Updated {getRelativeTime(guide.updatedAt)}
                    </Text>
                        ...
                        <>
                            ...
                            {guide.groups.map((group) => (
                                <View key={group.id} style={styles.groupRow}>
                                    <TouchableOpacity
                                        style={{ flex: 1 }}
                                        activeOpacity={0.7}
                                        onPress={() => { setActiveGuideId(guide.id); setActiveGroupId(group.id); setScreen('detail'); }}
                                    >
                                    ...
                                    <TouchableOpacity
                                        ...
                                        activeOpacity={0.7}
                                        onPress={() => { setMenuGuideId(guide.id); setMenuGroupId(group.id); openModal('groupMenu'); }}
                                    >
                                        <MoreVertical size={16} color="#6B7280" />
                                    </TouchableOpacity>
                                </View>
                            ))}
                            <TouchableOpacity style={styles.addGroupBtn} activeOpacity={0.7} onPress={() => addGroup(guide.id)}>
                                <Plus size={15} color="#2563EB" style={{ marginRight: 6 }} />
                            ...
                            <View style={styles.renameDeleteRow}>
                                <TouchableOpacity
                                    ...
                                    activeOpacity={0.7}
                                    onPress={() => { setMenuGuideId(guide.id); openModal('renameGuide', guide.name); }}
                                >
                                    <Pencil size={14} color="#374151" style={{ marginRight: 6 }} />
                                    <Text style={styles.renameBtnText}>Rename</Text>
                                </TouchableOpacity>
                                <TouchableOpacity style={styles.deleteBtn} activeOpacity={0.7} onPress={() => deleteGuide(guide.id)}>
...
const styles = StyleSheet.create({
    ...
    header: {
        ...
        paddingHorizontal: H_PADDING, paddingTop: 12, paddingBottom: 14,
        borderBottomWidth: 1, borderBottomColor: '#F1F2F4',
        backgroundColor: '#FFFFFF', marginTop: 18,
    },
    ...
    avatar: { width: 32, height: 32, borderRadius: 16, backgroundColor: '#E5E7EB', alignItems: 'center', justifyContent: 'center', marginRight: 12 },
    avatarText: { fontSize: 12, fontWeight: '700', color: '#374151' },
    cartButton: { width: 32, height: 32, alignItems: 'center', justifyContent: 'center' },
    cartBadge: { position: 'absolute', top: -4, right: -6, minWidth: 16, height: 16, borderRadius: 8, backgroundColor: '#2563EB', alignItems: 'center', justifyContent: 'center', paddingHorizontal: 3 },
    cartBadgeText: { fontSize: 10, fontWeight: '700', color: '#FFFFFF' },
    contentWrap: { flex: 1, paddingHorizontal: H_PADDING, paddingTop: 24 },
    emptyCard: { backgroundColor: '#FFFFFF', borderRadius: 20, borderWidth: 1, borderColor: '#F1F2F4', alignItems: 'center', paddingVertical: 44, paddingHorizontal: 24, marginTop: 80 },
    starCircle: { width: 72, height: 72, borderRadius: 36, backgroundColor: '#EFF6FF', alignItems: 'center', justifyContent: 'center', marginBottom: 20 },
    ...
    createButton: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#2563EB', paddingVertical: 14, paddingHorizontal: 28, borderRadius: 20 },
    createButtonText: { color: '#FFFFFF', fontWeight: '700', fontSize: 13 },
    detailScroll: { paddingHorizontal: H_PADDING, paddingTop: 16, paddingBottom: 40, backgroundColor: '#FFFFFF', flexGrow: 1 },
    breadcrumbRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 10 },
    backBtn: { width: 30, height: 30, borderRadius: 8, borderWidth: 1, borderColor: '#E5E7EB', alignItems: 'center', justifyContent: 'center', marginRight: 8 },
    ...
    detailTitleRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 14 },
    detailTitle: { fontSize: 20, fontWeight: '800', color: '#111827', marginRight: 10 },
    itemsBadge: { backgroundColor: '#F3F4F6', borderRadius: 20, paddingHorizontal: 10, paddingVertical: 3 },
    itemsBadgeText: { fontSize: 12, color: '#6B7280', fontWeight: '600' },
    detailActionRow: { flexDirection: 'row', gap: 10, marginBottom: 20 },
    detailActionBtn: { flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 8, paddingVertical: 9, paddingHorizontal: 12 },
    ...
    basketCircle: { width: 64, height: 64, borderRadius: 32, backgroundColor: '#F3F4F6', alignItems: 'center', justifyContent: 'center', marginBottom: 16 },
    detailEmptyTitle: { fontSize: 17, fontWeight: '800', color: '#111827', marginBottom: 8 },
    detailEmptySubtitle: { fontSize: 13, color: '#6B7280', textAlign: 'center', lineHeight: 20, marginBottom: 24 },
    addProductsBtn: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#2563EB', paddingVertical: 14, paddingHorizontal: 32, borderRadius: 30 },
    addProductsBtnText: { color: '#FFFFFF', fontWeight: '700', fontSize: 14 },
    listScroll: { paddingHorizontal: H_PADDING, paddingTop: 20, paddingBottom: 40 },
    ...
    listCreateBtn: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#2563EB', paddingVertical: 9, paddingHorizontal: 16, borderRadius: 10 },
    listCreateBtnText: { color: '#FFFFFF', fontWeight: '700', fontSize: 13 },
    guideCard: { backgroundColor: '#FFFFFF', borderRadius: 16, borderWidth: 1, borderColor: '#E5E7EB', padding: 16 },
    guideCardHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: 4 },
    guideCardName: { fontSize: 15, fontWeight: '700', color: '#111827', marginRight: 8 },
    defaultBadge: { backgroundColor: '#DBEAFE', borderRadius: 20, paddingHorizontal: 10, paddingVertical: 3 },
    defaultBadgeText: { fontSize: 11, fontWeight: '700', color: '#2563EB' },
    guideCardMeta: { fontSize: 12, color: '#9CA3AF', marginBottom: 4 },
    guideCardDivider: { height: 1, backgroundColor: '#F1F2F4', marginVertical: 12 },
    ...
    addGroupBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 11, borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 10, marginTop: 4, marginBottom: 4 },
    addGroupBtnText: { fontSize: 13, fontWeight: '600', color: '#2563EB' },
    renameDeleteRow: { flexDirection: 'row', gap: 10, marginBottom: 12 },
    renameBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 10, paddingVertical: 11 },
    renameBtnText: { fontSize: 13, fontWeight: '600', color: '#374151' },
    deleteBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#FFF1F2', borderRadius: 10, paddingVertical: 11 },
    deleteBtnText: { fontSize: 13, fontWeight: '600', color: '#EF4444' },
    downloadBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#2563EB', borderRadius: 10, paddingVertical: 14 },
    downloadBtnText: { color: '#FFFFFF', fontWeight: '700', fontSize: 14 },
    menuOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.15)', justifyContent: 'center', alignItems: 'center' },
    groupMenuCard: { backgroundColor: '#FFFFFF', borderRadius: 14, width: 180, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.12, shadowRadius: 10, elevation: 8, overflow: 'hidden' },
    groupMenuItem: { flexDirection: 'row', alignItems: 'center', paddingVertical: 14, paddingHorizontal: 16 },
    groupMenuItemText: { fontSize: 14, fontWeight: '600', color: '#374151' },
    groupMenuDivider: { height: 1, backgroundColor: '#F1F2F4' },
    modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.25)', justifyContent: 'center', alignItems: 'center', paddingHorizontal: 20 },
    modalCard: { width: '100%', maxWidth: 350, backgroundColor: '#FFFFFF', borderRadius: 18, padding: 18, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.15, shadowRadius: 10, elevation: 8 },
    ...
    textInput: { borderWidth: 1.5, borderColor: '#2563EB', borderRadius: 15, paddingVertical: 13, paddingHorizontal: 14, fontSize: 12, color: '#111827', marginBottom: 4, backgroundColor: '#FAFAFA' },
    textInputError: { borderColor: '#EF4444' },
    errorText: { fontSize: 12, color: '#EF4444', marginBottom: 8, marginLeft: 2 },
    saveButton: { backgroundColor: '#2563EB', borderRadius: 15, paddingVertical: 15, alignItems: 'center', marginTop: 16, marginBottom: 10 },
    saveButtonText: { color: '#FFFFFF', fontWeight: '700', fontSize: 14 },
    cancelButton: { borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 15, paddingVertical: 14, alignItems: 'center' },
